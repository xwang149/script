...
def msg_busytime_plot(APP, subplot):
    ...
    for item in range(len(APP.msg_busytime)):
        ...
        for i in range(len(busytime)):
            if (busytime[i] > 0 and yvals[i] < y_nozero):
                y_nozero = yvals[i]
                break
    ...
    title = APP.name + '-TerminalLinkSaturatedTime'
    # plt.title(title, fontsize = label_font)
...
def router_lch_traffic_plot(app, subplot):
    ...
    for item in range(len(app.router_lch_load)):
       print len(app.router_lch_load[item]), app.router_lch_load[item][0], app.router_lch_load[item][len(app.router_lch_load[item])-1]
       router_lch_load = [None]*(maxlen-len(app.router_lch_load[item]))+ \
                           sorted(app.router_lch_load[item])
       # print len(router_lch_load), router_lch_load[0], router_lch_load[len(router_lch_load)-1]
       plt.plot(router_lch_load, label=app.xlabel[item]
                ,linewidth=line_width, color=line_style[item][0],\
                linestyle=line_style[item][1]
    # for item in range(len(app.router_lch_load)):
    ...
    plt.locator_params(axis='x', nbins=5)
    plt.xlabel('Channel ID sorted by local channel traffic', fontsize=label_font)
...
def router_gch_traffic_plot(app, subplot):
    ...
    for item in range(len(app.router_gch_load)):
        ...
        router_gch_load = [None]*(maxlen-len(app.router_gch_load[item]))+ \
                            sorted(app.router_gch_load[item])
        # print router_gch_load
        plt.plot(router_gch_load, label=app.xlabel[item]
                ,linewidth=line_width, color=line_style[item][0],\
                linestyle=line_style[item][1]
                )
    ...
    plt.xlabel('Channel ID sorted by global channel traffic', fontsize=label_font)
    plt.ylabel('Traffic Amount(MB)', fontsize=label_font)
    if app.name in 'syn':
...
def router_lch_sat_plot(app, subplot):
    plt.figure(subplot)
    y_nozero = 1
    for item in range(len(app.router_lch_sat)):
        lch_sat = app.router_lch_sat[item]
        lch_sat.sort()
        yvals = np.arange(len(lch_sat))/float(len(lch_sat))
        for i in range(len(lch_sat)):
            if (lch_sat[i] > 0 and yvals[i] < y_nozero):
                y_nozero = yvals[i]
                break
        plt.plot(lch_sat, yvals*100, label=app.xlabel[item]
                ,linewidth=line_width
                ,linestyle=line_style[item][1]
                ,color=line_style[item][0]
                )
...
def router_gch_sat_plot(app, subplot):
    plt.figure(subplot)
    y_nozero = 1
    for item in range(len(app.router_gch_sat)):
        gch_sat = app.router_gch_sat[item]
        gch_sat.sort()
        yvals = np.arange(len(gch_sat))/float(len(gch_sat))
        for i in range(len(gch_sat)):
            if (gch_sat[i] > 0 and yvals[i] < y_nozero):
                y_nozero = yvals[i]
                break
        plt.plot(gch_sat, yvals*100, label=app.xlabel[item]
                ,linewidth=line_width
                ,linestyle=line_style[item][1]
                ,color=line_style[item][0]
                )
